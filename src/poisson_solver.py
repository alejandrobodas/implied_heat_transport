...
def calc_Ax(x, A_e, A_w, A_s, A_n, A_p):
    ...
    #     A_p[j-1, i-1]*x[j, i]
    Ax[1:M+1, 1:N+1] = A_s[0:M, 0:N] * x[0:M, 1:N+1] + A_w[0:M, 0:N] * x[1:M+1, 0:N] + \
               A_e[0:M, 0:N] * x[1:M+1, 2:N+2] + A_n[0:M, 0:N] * x[2:M+2, 1:N+1] + \
               A_p[0:M, 0:N] * x[1:M+1, 1:N+1]
    Ax = swap_bounds(Ax)
    return Ax
...
def precon_a(x, M_w, M_s, M_p, Cx):
    for j in range(1, M + 1):
        for i in range(1, N + 1):
            Cx[j, i] = M_p[j, i] * (x[j, i] - M_s[j, i] * Cx[j - 1, i] - M_w[j, i] * Cx[j, i - 1])
...
def precon_b(M_e, M_n, Cx):
    for j in range(M, 0, -1):
        for i in range(N, 0, -1):
            Cx[j, i] = Cx[j, i] - M_e[j, i] * Cx[j, i + 1] - M_n[j, i] * Cx[j + 1, i]
